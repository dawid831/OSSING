...
def link_usage_heatmap(a: np.ndarray, ax=None, *, collapse_n: bool | None = None):
    ...
    if ax is None:
        _, ax = plt.subplots(
            figsize=(1 + 0.6 * a.shape[-1], 0.3 * np.prod(a.shape[:-1]) + 2)
        )
    ...
    for i in range(grid.shape[0]):
        for j in range(grid.shape[1]):
            if grid[i, j]:
                ax.text(j, i, "1", ha="center", va="center", color="white", fontsize=7)
...
def _infer_link_role(a: np.ndarray) -> list[dict]:
    ...
    for l in range(L):
        ...
        if len(users_using) == 1 and len(servers_hit) == S:
            roles.append({"kind": "user_access", "user": next(iter(users_using))})
        elif len(servers_hit) == 1 and len(users_using) == M_plus_1:
            roles.append({"kind": "server_access", "server": next(iter(servers_hit))})
            ...
            roles.append(
                {"kind": "other", "users": users_using, "servers": servers_hit}
            )
    return roles
...
def _draw_multirouter(ax, a: np.ndarray, roles: list[dict]):
    ...
    for l, role in enumerate(roles):
        if role["kind"] == "user_router_access":
            ...
            _labeled_edge(ax, (user_x, user_y[u]), (router_x, router_y[r_idx]), f"L{l}")
        elif role["kind"] == "server_access":
            s = role["server"]
            r_idx = server_to_router.get(s, 0)
            _labeled_edge(ax, (router_x, router_y[r_idx]), (srv_x, srv_y[s]), f"L{l}")
    return ax
